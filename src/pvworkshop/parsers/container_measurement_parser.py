...
from pvworkshop.schema_packages.container_package import (
    container_EQEmeasurement,
    container_JVmeasurement,
    container_Measurement,
    container_SimpleMPPTracking,
)
...
class RawFilecontainer(EntryData):
    processed_archive = Quantity(
        type=Activity,
    )


class containerParser(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        entry = container_Measurement()

        if measurment_type == 'jv':
            entry = container_JVmeasurement()
        if measurment_type == 'eqe':
            entry = container_EQEmeasurement()
        if measurment_type == 'mppt':
            entry = container_SimpleMPPTracking()
        ...
        archive.data = RawFilecontainer(processed_archive=get_reference(archive.metadata.upload_id, eid))
