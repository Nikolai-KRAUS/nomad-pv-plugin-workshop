...
class RawFilefoo(EntryData):
    processed_archive = Quantity(
        type=Activity,
    )


class fooParser(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        entry = foo_Measurement()

        if measurment_type == 'jv':
            entry = foo_JVmeasurement()
        if measurment_type == 'eqe':
            entry = foo_EQEmeasurement()
        if measurment_type == 'mppt':
            entry = foo_SimpleMPPTracking()
        ...
        archive.data = RawFilefoo(processed_archive=get_reference(archive.metadata.upload_id, eid))
